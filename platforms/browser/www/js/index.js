ons.ready(function () {
    ...
    getUserLocation();
    navigator.geolocation.watchPosition(watchLocationSuccess, watchLocationError, {
        timeout: 25000,
        maximumAge: 0,
...
document.addEventListener('init', function (event) {
    ...
    if (page.id === 'page1') {
        ...
        //Wait for map page load before loading map
        initMap();
...
function addStoryMarker(markerLat, markerLng, storyName, storyTitle, storyAudio) {
    ...
    var newHotspot = {
        ...
        northEdge: northBounds,
        southEdge: southBounds,
        eastEdge: eastBounds,
        westEdge: westBounds,
        markerLat: markerLat,
        markerLng: markerLng
    ...
    function loadStoryPage(storyName, storyTitle, storyAudio) {
        ...
        //pass data to page4 and navigate there
        myNavigator.pushPage('page4.html', {
            data: {
                name: storyName,
                title: storyTitle,
                audio: storyAudio
    ...
    // createAlertDialog (Listen Notification) - Triggers when hotspot is entered /////
...
mediaRecorder.ondataavailable = function(blob) {
    var audioTag = document.getElementById("audioOutput");  // aud is the audio tag
...
mediaRecorder.start(60000);    // stop recording audio after 1 minute
}